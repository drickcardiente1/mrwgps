...
from api import update_user_settings
from api.models import AuthTransaction
from api.models import OTPValidation
...
def check_email(value: str, porpose: str) -> bool:
    if OTPValidation.objects.filter(destination=value, porpose = porpose).exists():
        return True
    else:
        return False
...
def validate_otp(value: str, otp: int) -> bool:
    try:
        ...
        otp_object: OTPValidation = OTPValidation.objects.get(destination=value, is_validated=False)
    except OTPValidation.DoesNotExist:
        raise NotFound(
            detail=_(
                "No pending OTP validation request found for provided "
                "Email. Kindly send an OTP first"
            )
        )
...
def check_otp(value: str, otp: int ) -> str:
    try:
        # Try to get OTP Object from Model and initialize data dictionary
        otp_object: OTPValidation = OTPValidation.objects.get(destination=value, is_validated=False)
        if str(otp_object.otp) == str(otp):
